...
impl<P> Envelope<P> where P: Point, <P as Point>::F: Float {
    ...
    pub fn y(&self, x: <P as Point>::F) -> <P as Point>::F {
        ...
        self.interpolate(x, self.points[i-1], self.points[i])
    }

    /// Return `value` for the given `time` clamped between the given values.
    #[inline]
    pub fn value_clamped_at(&self,
                            x: <P as Point>::F,
                            min: <P as Point>::F,
                            max: <P as Point>::F) -> <P as Point>::F {
        clamp(self.y(x), min, max)
    }
